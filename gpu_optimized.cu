...
__global__ void convolution(float *image, const float* __restrict__ kernel, int n , int m, int k,float *out_image)
{
        ...
        __shared__ float shared_image[TILESIZE+8][TILESIZE+8];

        int row_output = TILESIZE * blockIdx.y + ty;
        int col_output = TILESIZE * blockIdx.x + tx;

        int row_input= row_output-4;
        int col_input= col_output-4;
...
/******************************************************************************Fully_Connected Kernel******************************************************************************/
 ...
 end3 = std::chrono::system_clock::now();
  ...
  std::cout<<" For array size " << n <<" The time required for Matrix_Multiplication layer is "<<(totaltime3.count())<<std::endl;
